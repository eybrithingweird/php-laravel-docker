...
const BoxContentCancelled = ({token}) => {
	...
	// console.log(bufferEmployees);
	// console.log(user_id);
	// console.log(bufferEmployees.filter((emp) => emp.buffer_employee_id === user_id));
	...
	return (
		<>
			<SelectedLetterProvider>
						...
						<div className="table-responsive">
							...
							<div className="flex space-x-3 form-inline">
								...
								<select name="month" className="form-control" onChange={(e) => { setMonth(e.target.value); }}>
									<option value="00" selected={month == '00' ? true : false}>-Month-</option>
								...
								<button name="btnSearch" type="submit" className="btn btn-default" onClick={(e) => { e.preventDefault(); filterLettersCancelled(); }}>
									Search
								</button>

								<button name="btnClear" type="submit" className="btn btn-default" onClick={(e) => { e.preventDefault(); clearFiltersCancelled(); }}>
									Clear
								</button>
							</div>
						</div>
				...
				<div className="row">
					<TableResults type={type} letterForms={filteredLettersCancelled} isFetching={isFetchingCancelled && !isLoadingSearching} />
				</div>
			</SelectedLetterProvider>
		</>
	);
...
export default BoxContentCancelled;